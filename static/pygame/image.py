...
BMP images. When built with full image support, the _pygame_image.load()
...
def save(surface, file):
    ...
    if hasattr(file, 'write'):
        ...
        if fileext == '.bmp':
            ...
            raise _pygame_base.error('No support for jpg compiled in.') # TODO
        elif fileext == '.png':
            raise _pygame_base.error('No support for png compiled in.') # TODO
        else:
...
def tostring(surface, format, flipped=False):
    ...
    * ARGB, 32bit image with alpha channel first
    ...
    '''
    #brython
    return _pygame_surface.Surface.toDataURL('image/%s' % format)
...
def frombuffer(string, size, format):
    ...
    This method takes the same arguments as _pygame_image.fromstring(), but is
