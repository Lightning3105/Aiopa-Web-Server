...
controlled by flags passed to _pygame_display.set_mode().

Pygame can only have a single display active at any time. Creating a new one
with _pygame_display.set_mode() will close the previous display. If precise
control is needed over the pixel format or display resolutions, use the
functions _pygame_display.mode_ok(), _pygame_display.list_modes(), and
_pygame_display.Info() to query information about the display.
...
event queue. _pygame_QUIT is sent when the user has requested the program
to shutdown. The window will receive _pygame_ACTIVEEVENT events as the
display gains and loses input focus. If the display is set with the
_pygame_RESIZABLE flag, _pygame_VIDEORESIZE events will be sent when the
user adjusts the window dimensions. Hardware displays that draw direct
to the screen will get _pygame_VIDEOEXPOSE events when portions of the
...
from SDL import *
import _pygame_base
import _pygame_pkgdata
import _pygame_surface

#brython
import _pygame_constants
...
def __PYGAMEinit__():
    _pygame_base.register_quit(_display_autoquit)
...
def init():
    ...
    automatically when you call the higher level `_pygame_init`.
...
def set_mode(resolution, flags=0, depth=0):
    ...
    w, h = resolution
    if w <= 0 or h <= 0:
        raise _pygame_base.error('Cannot set 0 sized display mode')
    ...
    if _display_surface:
        ...
        #_display_surface = _pygame_surface.Surface(surf=surf)
        _display_surface = _pygame_surface.Surface(dim=(w,h))
    ...
    if sys.platform != 'darwin':
        if not _icon_was_set:
            try:
                file = _pygame_pkgdata.getResource(_icon_defaultname)
                iconsurf = _pygame_image.load(file)
...
def flip():
    ...
    is using the flags _pygame_HWSURFACE and _pygame_DOUBLEBUF, this will wait
    ...
    When using an _pygame_OPENGL display mode this will perform a gl buffer
    swap.
    '''
    pass
    _video_init_check()
    ...
    if not screen:
        raise _pygame_base.error('Display mode not set')
...
def update(*rectangle):
    ...
    This function is like an optimized version of _pygame_display.flip() for
    ...
    This call cannot be used on _pygame_OPENGL displays and will generate an
    exception.
    ...
    if screen.flags & SDL_OPENGL:
        raise _pygame_base.error('Cannot update an OPENGL display')
    ...
    if not rectangle:
        ...
        try:
            rect = _pygame_rect._rect_from_object(rectangle)._r
            rect = _crop_rect(w, h, rect)
            if rect:
                SDL_UpdateRect(screen, rect.x, rect.y, rect.w, rect.h)
        except TypeError:
            rectangle = rectangle[0]
            rects = [_crop_rect(w, h, _pygame_rect._rect_from_object(r)._r) \
...
class VideoInfo:
    ...
    def __init__(self):
        ...
        info=None
        if not info:
            raise _pygame_base.error('Could not retrieve video info')
...
def list_modes(depth=0, flags=_pygame_constants.FULLSCREEN):
    ...
    display.  The flags defaults to _pygame_FULLSCREEN, but you may need to add
...
def mode_ok(size, flags=0, depth=0):
    ...
    This function uses the same arguments as _pygame_display.set_mode(). It is
    ...
    The most useful flags to pass will be _pygame_HWSURFACE, _pygame_DOUBLEBUF, 
    and maybe _pygame_FULLSCREEN. The function will return 0 if these display
    flags cannot be set.
...
def gl_set_attribute(flag, value):
    ...
    When calling `_pygame_display.set_mode` with the OPENGL flag,
    ...
    This must be called before `_pygame_display.set_mode`.
...
def gl_get_attribute(flag):
    ...
    After calling `_pygame_display.set_mode` with the OPENGL flag
...
def get_active():
    ...
    done with the call to ``_pygame_display.set_mode()``. It is
...
def iconify():
    ...
    When the display is iconified _pygame_display.get_active() will return
    False. The event queue should receive a _pygame_APPACTIVE event when the
...
def toggle_fullscreen():
    ...
    it is better to call _pygame_display.set_mode() with new display flags.
...
def set_palette(palette=None):
    ...
    if not surf:
        raise _pygame_base.error('No display mode is set')
    if surf.format.BytesPerPixel != 1 or not surf.format._palette:
        raise _pygame_base.error('Display mode is not colormapped')
...
def _video_init_check():
    if not SDL_WasInit(SDL_INIT_VIDEO):
        raise _pygame_base.error('video system not initialized')
